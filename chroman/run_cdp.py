...
from chroman.chromium_downloader import chromium_executable
import context
...
async def make_request(targetlist, send_channel, receive_channel, sleeptime):
    """
    ...
    for targeturl, pdfpath in targetlist:
        await save_pdf(browser_url, targeturl, pdfpath, sleeptime)
    async with send_channel:
        logger.info("sending terminate message")
        await send_channel.send("terminate")


async def main(headless, get_addr, url, make_request, targetlist, sleeptime):
    """
    run functions in nursery

    Parameters
    ----------
    headless: function to run

    get_addr: function to run

    url: argument for get_addr

    make_request: function to run

    targetlist: argument for make_request
       
    sleeptime: int
       amount of time to sleep while waiting for webpage rendering
    ...
    async with trio.open_nursery() as nursery:
        ...
        nursery.start_soon(make_request, targetlist, send_channel_1, receive_channel, sleeptime)
    logger.info(f"through main")

def run(targetlist, sleeptime):
    """
    set up trio and chrome then run
